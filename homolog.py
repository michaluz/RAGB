...
class Homolog:
    ...
    def __init__(self, Query_accession, Query_common, Query_locus, Query_gene_name, Query_gene_id, Query_description, Query_start, Query_stop, Query_strand, Query_gc, Accession, Organism, Locus, Gene_name, Description, Start, Stop, Strand, GC, Percent_ident, Aligned_length, Number_mismatched, Number_gaps, Align_query_start, Align_query_stop, Align_subject_start, Align_subject_stop, Eval, Bits_score):
        ...
        self.__query_gene_name = str(Query_gene_name)
        self.__query_gene_id = str(Query_gene_id)
        self.__query_description = str(Query_description)
        ...
        self.__gene_name = str(Gene_name)
    ...
    def from_blast(cls, line):
        ...
        Query_accession, Query_common, Query_locus, BLAST_annotation,Query_description,Query_start, Query_stop, Query_strand, Query_type, Synonyms, Query_gc, Accession, Organism, Locus, Genbank_annotation,Description, Start, Stop, Strand, GC, Percent_ident, Aligned_length, Number_mismatched, Number_gaps, Align_query_start, Align_query_stop, Align_subject_start, Align_subject_stop, Eval, Bits_score
        '''
        try:
            query_line, subject_line, percent_ident, aligned_length, number_mismatched, number_gaps, align_query_start, align_query_stop, align_subject_start, \
            align_subject_stop, Eval, bits_score = line.strip().split('\t')
    ...
    #Query_accession, Query_common, Query_locus, Query_gene_name, Query_gene_id, Query_description, Query_start, Query_stop, Query_strand, Query_gc, Accession, Organism, Locus, Gene_name, Description, Start, Stop, Strand, GC, Percent_ident, Aligned_length, Number_mismatched, Number_gaps, Align_query_start, Align_query_stop, Align_subject_start, Align_subject_stop, Eval, Bits_score
    ...
    def query_description(self):
        return str(self.__query_description)

    def query_gene_name(self):
        return str(self.__query_gene_name)

    def query_gene_id(self):
        return str(self.__query_gene_id)
    ...
    def ret_str(self, delim = '\t'):
        return delim.join([str(i) for i in[self.query_accession(), self.query_common(), self.query_locus(), self.query_description, self.query_gene_name(), self.query_gene_id(), self.query_start(), self.query_stop(), self.query_strand(), \
            self.query_gc(), self.accession(), self.organism(), self.locus(), self.gene_name(), self.start(), self.stop(), self.strand(), \
    ...
    def ReturnVals(self): # I DO NOT LIKE THIS... 
        return self.query_accession(), self.query_common(), self.query_locus(), self.query_gene_name(), self.query_gene_id(), self.query_description(), self.query_start(), self.query_stop(), self.query_strand(), self.query_type(),  self.synonyms(), self.query_gc(), self.accession(), self.organism(), self.locus(), self.gene_name(), self.start(), self.stop(), self.strand(), self.gc(), self.percent_ident(), self.aligned_length(), self.align_query_start(), self.align_query_stop(), self.align_subject_start(), self.align_subject_stop(),self.e_val(), self.bits_score()
